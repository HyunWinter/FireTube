...
class PlaylistAdapter(context : Context, playlists : ArrayList<Playlist>) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        var channel: Playlist = this.mPlayLists!![position]

        holder.title.text = channel.title
        Glide.with(mContext!!).load(channel.thumbnail).into(holder.thumbnail)
    }
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var title = itemView.findViewById<View>(R.id.Channel_Title) as TextView
        var thumbnail = itemView.findViewById(R.id.Channel_Thumbnail) as ImageView
    }
}
